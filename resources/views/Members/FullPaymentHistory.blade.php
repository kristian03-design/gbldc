...
<head>
  ...
  <style>
    ...
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .stat-value { font-size: 18px; font-weight: 700; color: var(--ink); line-height: 1.2; }
    ...
    tbody td { padding: 13px 16px; font-size: 13px; color: var(--ink); vertical-align: middle; }
    .amount { font-weight: 700; }
    .ref-num { font-family: 'Courier New', monospace; font-size: 11px; color: var(--muted); letter-spacing: .03em; }
    .remarks-text { font-size: 12px; color: var(--muted); max-width: 160px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
    ...
    .badge-loan      { background: #f0fdf4; color: #16a34a; }
    .badge-loan::before      { background: #16a34a; }
    .badge-capital   { background: #eff6ff; color: #1d4ed8; }
    .badge-capital::before   { background: #2563eb; }
    .badge-other-type{ background: #f3f4f6; color: var(--muted); }
    .badge-other-type::before{ background: #9ca3af; }
  ...
  <div class="page-body">
    ...
    @php
      ...
      foreach ($allPayments as $p) {
        if ($p->transaction_type == 'Loan Payment')   $totalLoanPayments  += $p->payment_amount;
        elseif ($p->transaction_type == 'Shared Capital') $totalSharedCapital += $p->payment_amount;
      }
      $grandTotal = $totalLoanPayments + $totalSharedCapital;
    @endphp

    {{-- ── Stats ── --}}
    <div class="stats-row">
      <div class="stat-card">
        <div class="stat-icon amber"><i data-lucide="receipt"></i></div>
        <div>
      ...
      <div class="stat-card">
        ...
        <div>
          ...
          <div class="stat-value">₱{{ number_format($totalLoanPayments, 2) }}</div>
        </div>
      </div>
      <div class="stat-card">
        <div class="stat-icon blue"><i data-lucide="piggy-bank"></i></div>
        <div>
          <div class="stat-label">Shared Capital</div>
          <div class="stat-value">₱{{ number_format($totalSharedCapital, 2) }}</div>
        </div>
      </div>
      <div class="stat-card">
    ...
    <div class="filter-row">
      ...
      <button class="filter-btn" onclick="filterTable('Loan Payment', this)">Loan Payments</button>
      <button class="filter-btn" onclick="filterTable('Shared Capital', this)">Shared Capital</button>
      <div class="filter-spacer"></div>
      <div class="search-wrap">
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
          <circle cx="11" cy="11" r="8"/><path d="m21 21-4.35-4.35"/>
        </svg>
        <input type="text" id="table-search" class="search-input" placeholder="Search transactions…">
      </div>
    </div>
    ...
    <div class="table-card">
      <div class="table-card-header">
        <h2><i data-lucide="list-ordered"></i> All Payment Transactions</h2>
        <span id="row-count" style="font-size:12px;color:var(--muted);font-weight:600;">
            ...
            <tr data-type="{{ $payment->transaction_type }}">
              <td>
                @if($payment->transaction_type == 'Loan Payment')
                  <span class="badge badge-loan">Loan Payment</span>
                @elseif($payment->transaction_type == 'Shared Capital')
                  <span class="badge badge-capital">Shared Capital</span>
                @else
                  <span class="badge badge-other-type">{{ $payment->transaction_type }}</span>
                @endif
              </td>
              <td><span class="amount">₱{{ number_format($payment->payment_amount, 2) }}</span></td>
              ...
              <td>
                ...
                @else
                  <span class="badge badge-failed">{{ ucfirst($payment->payment_status) }}</span>
                @endif
              </td>
