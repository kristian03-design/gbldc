...
<head>
  ...
  <style>
    ...
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .modal-btn.sky      { background: var(--sky); color: #fff; }
    .modal-btn.sky:hover { background: #2563eb; }
...
<aside class="sidebar" id="sidebar">
  ...
  <nav class="sidebar-nav">
    ...
    <a href="http://localhost:8000/GBLDC-Member-Loan-Dashboard" class="nav-item active">
  ...
  <div class="page-body">
    ...
    <div class="welcome-banner">
      <div style="position:relative;z-index:1;">
        <h2>Hello, {{$fist_name}} {{$last_name}} ! <i data-lucide="smile"></i></h2>
      ...
      <div class="card">
        ...
        <div class="card-body">
          <div class="detail-grid">
            ...
            <div class="detail-item">
              <label>Interest Rate <span style="font-weight:400;color:var(--muted);font-size:11px;"></span></label>
              <div class="value">{{ (isset($loanInfo->interest_rate) && $loanInfo->interest_rate !== '' && $loanInfo->interest_rate !== null) ? number_format((float) $loanInfo->interest_rate, 1) . '%' : number_format(5, 1) . '%' }}</div>
            </div>
            <div class="detail-item">
    ...
    <div class="card" style="margin-bottom:20px;">
      ...
      <div class="card-body">
        @php
          ...
          // Compute Next Due Date for Loan Payment
          $nextLoanDueDateStr = '—';
          if ($loanInfo && isset($loanPaymentStartDate) && $loanInfo->frequency_of_payment) {
              ...
              if ($baseLoanDate) {
                  ...
                  if (str_contains($loanFreqStr, 'monthly')) {
                      $ld->addMonth();
                  } elseif (str_contains($loanFreqStr, 'weekly')) {
                      $ld->addWeek();
                  } elseif (str_contains($loanFreqStr, 'yearly') || str_contains($loanFreqStr, 'annually')) {
                      $ld->addYear();
                  }
          ...
          // Compute Next Due Date for Shared Capital
          $nextSharedDueDateStr = '—';
          if ($sharedCapitalInfo && isset($sharedCapitalPaymentStartDate) && $sharedCapitalInfo->payment_frequency) {
              ...
              if ($baseSharedDate) {
                  ...
                  if (str_contains($sharedFreqStr, 'monthly')) {
                      $sd->addMonth();
                  } elseif (str_contains($sharedFreqStr, 'weekly')) {
                      $sd->addWeek();
                  } elseif (str_contains($sharedFreqStr, 'yearly') || str_contains($sharedFreqStr, 'annually')) {
                      $sd->addYear();
                  }
        ...
        <div class="repay-grid">
          <div class="repay-card" id="loan-payment-div"
            ...
            @php
              ...
              if (isset($loanInfo?->monthly_payment) && $loanInfo->monthly_payment !== '' && $loanInfo->monthly_payment !== null) {
            ...
            <div class="repay-sub" style="margin-top: 4px; color: var(--primary);">Next Due: <strong>{{ $nextLoanDueDateStr }}</strong></div>
          </div>
          <div class="repay-card" id="shared-capital-div"
            ...
            <div class="repay-sub" style="margin-top: 4px; color: var(--primary);">Next Due: <strong>{{ $nextSharedDueDateStr }}</strong></div>
          </div>
        </div>
    ...
    <div class="card">
      ...
      <div style="overflow-x:auto;">
        <table class="pay-table" id="paymentHistoryTable">
          <thead><tr>
            <th>Type</th>
            <th>Amount</th>
            <th>Method</th>
...
<div class="modal-overlay" id="qrCodeModal">
  <div class="modal">
    <div class="modal-icon green"><i data-lucide="qr-code"></i></div>
    <h3>GCash Payment QR Code</h3>
    <p>Scan this QR code to make a payment via GCash.</p>
    <div style="display:flex;justify-content:center;margin-bottom:20px;" id="qrCodeContainer">
...
@endphp
        <img id="qrImage" src="{{ $qrSrc }}"
          alt="GCash QR Code"
          style="width:220px;height:220px;object-fit:contain;border-radius:12px;border:1px solid var(--border);">
      @else
        <div style="text-align:center;padding:32px 0;color:var(--muted);">
          <i data-lucide="qr-code" style="width:48px;height:48px;opacity:.3;display:block;margin:0 auto 10px;"></i>
          <p style="font-size:14px;">No QR Code available</p>
          <p style="font-size:12px;">Please contact admin to upload a QR code</p>
        </div>
      @endif
    </div>
    <div class="modal-btn-row">
      <button class="modal-btn cancel" id="closeQrModal">Close</button>
    </div>
  </div>
</div>
...
<script>
  ...
  // Payment / QR modal
  const makePaymentBtn2 = document.getElementById('makePaymentBtn2');
  const closeQrModal = document.getElementById('closeQrModal');
  
  if (makePaymentBtn2) makePaymentBtn2.addEventListener('click', () => openModal('qrCodeModal'));
  if (closeQrModal) closeQrModal.addEventListener('click', () => closeModal('qrCodeModal'));
