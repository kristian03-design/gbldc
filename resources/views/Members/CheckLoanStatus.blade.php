...
<head>
  ...
  <style>
    ...
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    /* ── Guarantor cards ── */
    .guarantor-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 16px; }
    .guarantor-card {
    ...
    .guarantor-card h4 i[data-lucide] { width: 13px; height: 13px; color: var(--emerald); }
...
<aside class="sidebar" id="sidebar">
  ...
  <nav class="sidebar-nav">
    ...
    <a href="{{ route('Member.Check.Loan.Status') }}" class="nav-item active">
...
<div class="main">
  ...
  <header class="topbar">
    ...
    <nav class="breadcrumb" aria-label="Breadcrumb">
      ...
      <span class="sep"><i data-lucide="chevron-right" style="width:12px;height:12px;"></i></span>
      <span class="current">Check Loan Status</span>
    </nav>
  </header>
  ...
  <div class="page-body">
    ...
    <div class="card">
      <div class="card-header">
        <h2><i data-lucide="activity"></i> Current Loan Status</h2>
        @php $s = strtolower($currentLoan->loan_status); @endphp
        <span class="status-badge {{ str_contains($s,'active') ? 'active' : (str_contains($s,'pending') ? 'pending' : (str_contains($s,'approved') ? 'approved' : 'default')) }}">
      ...
      <div class="card-body">
        <div class="progress-wrapper">
          ...
          <div class="progress-labels">
            ...
            <span>₱{{ number_format($currentLoan->loan_amount, 2) }} total</span>
          </div>
        </div>
      </div>
    </div>
    ...
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:20px;margin-bottom:20px;">
      ...
      <div class="card" style="margin-bottom:0;">
        <div class="card-header">
          <h2><i data-lucide="file-text"></i> Loan Information</h2>
        </div>
        <div class="card-body">
          <div class="info-list">
            <div class="info-row">
              <span class="lbl">Loan Number</span>
              <span class="val">{{ $currentLoan->loan_number }}</span>
            </div>
            <div class="info-row">
              <span class="lbl">Loan Type</span>
              <span class="val">{{ $currentLoan->loan_type }}</span>
            </div>
            <div class="info-row">
              <span class="lbl">Loan Amount</span>
              <span class="val green">₱{{ number_format($currentLoan->loan_amount, 2) }}</span>
            </div>
            <div class="info-row">
              <span class="lbl">Current Balance</span>
              <span class="val red">₱{{ number_format($currentLoan->loan_balance, 2) }}</span>
            </div>
            <div class="info-row">
              <span class="lbl">Loan Term</span>
              <span class="val">{{ $currentLoan->loan_term }} months</span>
            </div>
            <div class="info-row">
              <span class="lbl">Interest Rate <span style="font-size:11px;opacity:.8;">(p.a., compound by amount tier)</span></span>
              <span class="val">{{ (isset($currentLoan->interest_rate) && $currentLoan->interest_rate !== '' && $currentLoan->interest_rate !== null) ? number_format((float) $currentLoan->interest_rate, 1) . '%' : number_format(5, 1) . '%' }}</span>
            </div>
            <div class="info-row">
              <span class="lbl">Payment Frequency</span>
              <span class="val">{{ $currentLoan->frequency_of_payment }}</span>
            </div>
            <div class="info-row">
              <span class="lbl">Due Amount</span>
              <span class="val">₱{{ number_format($currentLoan->due_amount, 2) }}</span>
            </div>
            <div class="info-row">
              <span class="lbl">Payment Start Date</span>
              <span class="val">{{ $currentLoan->payment_start_date ? date('M d, Y', strtotime($currentLoan->payment_start_date)) : '—' }}</span>
            </div>
          </div>
        </div>
      </div>
        ...
        <div class="card-body">
          <div class="info-list">
            <div class="info-row block-row">
              <span class="lbl">Purpose of Loan</span>
              <span class="val">{{ $currentLoan->purpose_of_loan ?? 'Not specified' }}</span>
            </div>
            <div class="info-row">
              <span class="lbl">Date Applied</span>
              <span class="val">{{ $currentLoan->created_at ? $currentLoan->created_at->format('M d, Y') : 'N/A' }}</span>
            </div>
            <div class="info-row">
              <span class="lbl">Approved By</span>
              <span class="val">{{ $currentLoan->approved_by ?? 'Pending' }}</span>
            </div>
            <div class="info-row">
              <span class="lbl">Encoded By</span>
              <span class="val">{{ $currentLoan->encoded_by ?? 'N/A' }}</span>
            </div>
          </div>
        </div>
      </div>
    ...
    <div class="card">
      <div class="card-header">
        <h2><i data-lucide="users"></i> Guarantor Information</h2>
      </div>
      <div class="card-body">
        <div class="guarantor-grid">
          <div class="guarantor-card">
            <h4><i data-lucide="user"></i> First Guarantor</h4>
            <div class="info-list">
              <div class="info-row">
                <span class="lbl">Name</span>
                <span class="val">{{ $currentLoan->g1_fullname ?? '—' }}</span>
              </div>
              <div class="info-row">
                <span class="lbl">Relationship</span>
                <span class="val">{{ $currentLoan->g1_relationship ?? '—' }}</span>
              </div>
              <div class="info-row">
                <span class="lbl">Contact</span>
                <span class="val">{{ $currentLoan->g1_contact_number ?? '—' }}</span>
              </div>
            </div>
          </div>
          <div class="guarantor-card">
            <h4><i data-lucide="user"></i> Second Guarantor</h4>
            <div class="info-list">
              <div class="info-row">
                <span class="lbl">Name</span>
                <span class="val">{{ $currentLoan->g2_fullname ?? '—' }}</span>
              </div>
              <div class="info-row">
                <span class="lbl">Relationship</span>
                <span class="val">{{ $currentLoan->g2_relationship ?? '—' }}</span>
              </div>
              <div class="info-row">
                <span class="lbl">Contact</span>
                <span class="val">{{ $currentLoan->g2_contact_number ?? '—' }}</span>
              </div>
            </div>
          </div>
        </div>
    ...
    <div class="card">
      <div class="card-header">
        <h2><i data-lucide="receipt"></i> Payment History for This Loan</h2>
      </div>
    ...
    <div class="action-row">
      <a href="{{ route('Payment.Schedule', ['type' => 'loan']) }}" class="btn green">
    ...
    <div class="card">
      <div class="empty-state">
        ...
        <p>You currently don't have any active loans. All your loans may have been fully paid, or you haven't applied for one yet.</p>
        <div class="empty-actions">
          <a href="{{ route('Member.Loan.History') }}" class="btn outline">
            <i data-lucide="history"></i> View Loan History
          </a>
          <a href="{{ route('Redirecting.LoanApp', ['account' => $member_id], ['account' => $member_id]) }}" class="btn sky">
            <i data-lucide="plus"></i> Apply for New Loan
          </a>
        </div>
      </div>
    </div>
