...
<head>
  <meta charset="UTF-8" />
  <title>Loan Applications | GBLDC Admin</title>
  ...
  <style>
    ...
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    /* Lucide icons rendered via data-lucide in nav-item */
    ...
    /* Both svg and lucide i tags in h3 */
    ...
    .loan-type-pill svg,
    .loan-type-pill i[data-lucide] { width: 10px; height: 10px; }

    .amount-cell { font-weight: 600; color: var(--forest-mid) !important; }

    /* ── Action buttons — matching membership reference ── */
  ...
  <div class="topbar-left" style="display:flex; align-items:center;">
    <button class="mobile-toggle" id="mobile-toggle" onclick="document.getElementById('sidebar').classList.add('open'); document.getElementById('sidebar-overlay').classList.add('show');" style="margin-right:12px;">
      ...
      <div class="topbar-title">
        ...
        <p>Review, approve, or reject pending loan applications</p>
      </div>
    </div>
  </header>
  ...
  <div class="page-body">
    ...
    <div class="page-header">
      <div class="page-header-text">
        ...
        <p>Review pending applications and take action on each submission.</p>
      </div>
    ...
    <div class="table-card">
      <div class="table-card-header">
        ...
        <span class="count-badge">Awaiting Review</span>
      </div>
      ...
      <div style="overflow-x:auto;">
        <table id="loanTable" class="display responsive nowrap no-footer" style="width:100%">
          ...
          <tbody>
            ...
            <tr>
              <td>
                <div class="applicant-name">{{$LoanApplication->last_name}}, {{$LoanApplication->first_name}} {{$LoanApplication->middle_name}}</div>
              </td>
              <td class="td-muted">{{$LoanApplication->contact_number}}</td>
              <td>
                <span class="loan-type-pill">
                  <i data-lucide="tag" style="width:10px;height:10px;"></i>
                  {{$LoanApplication->loan_type}}
                </span>
              </td>
              <td class="amount-cell">₱{{ number_format($LoanApplication->loan_amount, 2) }}</td>
              <td class="td-muted">{{ date('M d, Y', strtotime($LoanApplication->created_at)) }}</td>
              <td>
                <div class="action-group">
                  <!-- View Details -->
                  <a href="{{route('Review.LoanApp', $LoanApplication->id)}}" class="action-btn view">
...
<script>
  ...
  // DataTable — re-run createIcons after every draw so icons render on paginated rows
